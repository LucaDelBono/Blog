            ...
            <div class="w-full bg-yellow-200 rounded-lg shadow border md:mt-0 sm:max-w-md xl:p-0 border-gray-700">
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    <h1 class="text-xl leading-tight tracking-tight text-gray md:text-2xl">
                        Una volta eliminato l'account non sarà possibile riattivarlo. Inoltre tutte le informazioni legate
                        all'account verranno eliminate.
                    </h1>

                    <form action="{{ route('userSettingsDelete.destroy', $user->id) }}" method="post">
                        @csrf
                        @method('delete')
